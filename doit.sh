...
do_opensuse()
{
    ...
    for i in i586 x86_64; do
      cat output/dvd-1.$i.list output/dvd-2.$i.list output/dvd-3.$i.list | LC_ALL=C sort -u > output/dvd-$i.list
    done
    ...
    for arch in i586 x86_64; do
       diff -u output/kde4_cd.$arch.list output/kde4_cd-default.$arch.list | grep -v +++ | grep ^+
       diff -u output/gnome_cd.$arch.list output/gnome_cd-default.$arch.list | grep -v +++ | grep ^+
    done
    ...
    for arch in i586 x86_64; do
	  ...
	  grep -vx $i output/$f.$arch.list > t && mv t output/$f.$arch.list
	done
        grep -v patterns-openSUSE output/$f.$arch.list > t && mv t output/$f.$arch.list
      done
      grep -vx openSUSE-release-ftp output/x11_cd-initrd.$arch.list > t && mv t output/x11_cd-initrd.$arch.list
    done
