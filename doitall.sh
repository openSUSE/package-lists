...
if test -z "$diffonly" || test -d "$diffonly"; then
   ...
   for i in `grep "package name=" osc/openSUSE\:Factory/_product/FROZEN.group | cut -d\" -f2`; do
      echo "<lock package='$i'/>" >> output/frozen.xml
   done
   cd testtrack/
...
for arch in i586 x86_64; do
  for f in gnome_cd-default gnome_cd kde4_cd-default kde4_cd gnome_cd-x11-default kde4_cd-base-default x11_cd; do
     if ! diff -u saved/$f.$arch.list output/$f.$arch.list ; then
        diff=1 
        break
     fi
     test "$diff" = 0 || break
...
if test "$diff" = 1; then
   echo "no diff"
   tar cjf /package_lists/filelists.tar.bz2 output/*_cd*.list
   cp output/*.list saved
fi

set -e

./check_yast.sh output/dvd-i586.list __i386__
./check_yast.sh output/dvd-x86_64.list __x86_64__
./check_yast.sh output/dvd-ppc.list __powerpc__

(
./check_size.sh output/dvd-i586.list i586
./check_size.sh output/dvd-x86_64.list x86_64
./check_size.sh output/dvd-ppc.list ppc
) | tee sizes

./mk_group.sh output/dvd-ppc.list DVD-ppc osc/openSUSE\:Factory/_product/DVD5-ppc.group only_ppc
./mk_group.sh output/dvd-i586.list DVD-i586 osc/openSUSE\:Factory/_product/DVD5-i586.group only_i586
./mk_group.sh output/dvd-x86_64.list DVD-x86_64 osc/openSUSE\:Factory/_product/DVD5-x86_64.group only_x86_64
./mk_group.sh output/promo_dvd.i586.list REST-DVD-promo-i386 osc/openSUSE\:Factory/_product/DVD5-promo-i386.group
./mk_group.sh output/langaddon-all.list REST-DVD osc/openSUSE\:Factory/_product/DVD5-lang.group
...
./mk_group.sh output/x11_cd.x86_64.list DVD osc/system:install:head/_product/DVD.group
